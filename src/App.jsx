...
const StickyNote = ({ initialX, initialY, initialText, color, onRemove }) => {
  ...
  return (
    <>
      ...
      <div className="fixed pointer-events-none z-[9999]" style={{ transform: `translate(${pos.x - 18}px, ${pos.y - 18}px)` }}>
        <img src={isCyber ? ASSETS.cyberRabbit : ASSETS.rabbitIcon} className="w-9 h-9 filter drop-shadow-[0_0_15px_rgba(168,234,255,0.4)] animate-os-rabbit-vibe" alt="" />
      </div>
    </>
  );
};
...
const Desktop = ({ bgm }) => {
  ...
  const [memos, setMemos] = useState([{ id: 1, initialX: 50, initialY: 350, initialText: "\u306B\u3093\u3058\u3093\uFF13\u672C", color: "bg-yellow-200/30" }]);
  ...
  const addNotify = useCallback((msg) => {
    const id = Date.now();
    setNotifications((p) => [...p, { id, msg }]);
    setTimeout(() => setNotifications((p) => p.filter((n) => n.id !== id)), 3000);
  }, []);

  const resetDreamTimer = useCallback(() => {
    setIsDreaming(false);
    clearTimeout(dreamTimer.current);
    dreamTimer.current = setTimeout(() => setIsDreaming(true), 60000);
  }, []);

  useEffect(() => {
    ["mousemove", "keydown", "touchstart", "mousedown"].forEach((e) =>
      window.addEventListener(e, resetDreamTimer)
    );
    resetDreamTimer();
    const getTimeGreeting = () => {
      const h = new Date().getHours();
      if (h >= 5 && h < 11) return "\u8D77\u304D\u305F";
      if (h >= 11 && h < 18) return "\u304A\u304B\u3048\u308A\uFF01";
      if (h >= 18 && h < 22) return "\u304A\u3064\u304B\u308C\u3055\u307E";
      return "\u307E\u3063\u305F\u308A\u301C";
    };
    setTimeout(() => addNotify(getTimeGreeting()), 1200);
    return () => {
      ["mousemove", "keydown", "touchstart", "mousedown"].forEach((e) =>
        window.removeEventListener(e, resetDreamTimer)
      );
    };
  }, [resetDreamTimer, addNotify]);
  ...
  return (
    <div className={`fixed inset-0 w-full h-full overflow-hidden select-none bg-black transition-all duration-1000 ${isRestarting ? "opacity-0 scale-95 blur-xl" : "opacity-100 scale-100 blur-0"}`}>
      ...
      {/* Notifications */}
      <div className="fixed top-12 left-0 right-0 z-[6000] flex flex-col items-center gap-3 pointer-events-none px-6">
        {notifications.map((n) => (
      ...
      {/* Sticky Notes */}
      {memos.map((m) => (
        <StickyNote key={m.id} initialX={m.initialX} initialY={m.initialY} initialText={m.initialText} color={m.color} onRemove={() => setMemos((p) => p.filter((note) => note.id !== m.id))} />
      ))}
      ...
      <div
        ...
        style={{ backgroundImage: `url(${WALLPAPERS[bgIndex]})`, filter: `brightness(${isCyber ? 0.35 : 0.5}) saturate(1.2)` }}
      >
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <span className="text-[9px] font-mono text-white/20 tracking-widest hidden sm:inline">ID: {userName}</span>
        </div>
      ...
      <div
        ...
        } hidden md:block pointer-events-none z-10 ${openApps.length > 0 ? "opacity-10 scale-90" : ""}`}
      >
      ...
      >
        ...
        >
          ...
          <button
            ...
            className={`${isMobile ? "w-10 h-10" : "w-14 h-14"} rounded-xl flex items-center justify-center transition-all duration-300 ${isSettingsOpen ? "bg-white/10 text-white" : "text-white/30 hover:text-white/60"}`}
          >
            <Sliders size={isMobile ? 18 : 22} strokeWidth={1.5} />
          </button>
          <button
            onClick={() => setIsPowerMenuOpen(true)}
            className={`${isMobile ? "w-10 h-10" : "w-14 h-14"} rounded-xl flex items-center justify-center text-rose-500/40 hover:text-rose-500 transition-all duration-300`}
          >
            <Power size={isMobile ? 18 : 22} strokeWidth={1.5} />
          </button>
        </div>
      </div>
      ...
      <div className={`fixed right-6 top-[15%] bottom-[20%] w-16 z-[2000] flex flex-col gap-5 p-3 bg-black/40 backdrop-blur-3xl border border-white/10 rounded-full transition-all duration-700 overflow-y-auto scrollbar-hide ${isSettingsOpen ? "translate-x-0 opacity-100" : "translate-x-24 opacity-0 pointer-events-none"}`}>
        <div className="flex flex-col gap-4 py-2">
          {Object.keys(THEMES).map((k) => (
            <button key={k} onClick={() => { setTheme(k); addNotify(`${k.toUpperCase()}_MODE`); }} className={`w-5 h-5 rounded-full mx-auto transition-all ${theme === k ? "scale-150 ring-4 ring-white/10" : "opacity-20 hover:opacity-100"}`} style={{ background: THEMES[k].hex }} />
          ))}
        </div>
        <div className="h-[1.5px] bg-white/10 shrink-0" />
        <div className="flex flex-col gap-4 py-2">
          {RIPPLE_MODES.map((m) => (
            <button key={m.id} onClick={() => { setRippleMode(m.id); addNotify(`RIPPLE: ${m.name}`); }} className={`p-2 rounded-full mx-auto transition-all ${rippleMode === m.id ? "bg-white/15 text-white" : "text-white/20 hover:text-white/40"}`}>
              <m.icon size={20} strokeWidth={1} />
            </button>
          ))}
        </div>
        <div className="h-[1.5px] bg-white/10 shrink-0" />
